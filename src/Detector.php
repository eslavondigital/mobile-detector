...
namespace Eslavon\Mobile;


class Detector
{

    private static array $signature = array("1207", "3gso", "4thp", "501i", "502i", "503i", "504i", "505i", "506i",
    ...
    public static function run()
    {
        ...
        $result[] = self::isUserAgent($user_agent);
        $result[] = self::isHttpProfile();
        $result[] = self::isWap($accept);
        $result[] = self::isSignature($user_agent);
        return in_array(true, $result);
    }

    private static function isUserAgent(string $user_agent): bool
    {
    ...
    private static function isHttpProfile(): bool
    {
    ...
    private static function isWap(string $accept): bool
    {
        if ((strpos($accept, 'text/vnd.wap.wml') !==false) or (strpos($accept,'application/vnd.wap.xhtml+xml') !== false)) {
            return true;
        } else {
            return false;
        }
    ...
    private static function isSignature(string $user_agent): bool
    {
        if (in_array( substr($user_agent,0,4), self::$signature)) {
            return true;
        } else {
            return false;
        }
